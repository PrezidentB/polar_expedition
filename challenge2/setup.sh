...
Équipe :
  ...
  - Identité inconnue — Membre mystérieux (non répertorié)
...
Nom : [CLASSIFIÉ]
Affiliation : Institut de Recherche Arctique International
...
→ Utilisez maintenant le CODE_LABO pour entrer dans le laboratoire
  et découvrir la vérité sur l'anomalie.
EOF

# --- Variable d'environnement : injectée pour tous les shells ---
echo 'export CODE_LABO="GLACE_PROFONDE_340"' >> /etc/environment
echo 'export CODE_LABO="GLACE_PROFONDE_340"' >> /home/player/.bashrc
echo 'export HINT_EREBUS="Les secrets sont parfois invisibles à première vue"' >> /home/player/.bashrc

# --- Laboratoire : récompense finale, accessible après le flag ---
cat > /home/player/base_erebus/laboratoire/ACCES_RESTREINT.txt << 'EOF'
[ LABORATOIRE PRINCIPAL — ACCÈS RESTREINT ]

Ce laboratoire ne peut être ouvert qu'avec le bon code.
Trouvez la variable d'environnement CODE_LABO et utilisez-la
pour valider votre mission.
EOF
